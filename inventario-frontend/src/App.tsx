...
function App() {
    ...
    auth.events.addSilentRenewError(() => {
        console.log(`Silent Renew Error. Redirecting to sistema ${window.REACT_APP_BASE_URL}/session-expired`);
        auth.signoutRedirect({
            id_token_hint: auth.user?.id_token,
            post_logout_redirect_uri: `${window.REACT_APP_BASE_URL}/session-expired`
        })
    });
